...
FROM models JOIN brands USING (brand_id)
...
SELECT *
FROM models
WHERE model_id BETWEEN 15 and 24;
...
FROM models LEFT JOIN awards ON (models.model_id = awards.winner_id)
...
FROM brands as b JOIN models as m ON (b.brand_id = m.brand_id)
ORDER BY b.name;
...
FROM brands FULL OUTER JOIN models USING (brand_id)
...
SELECT brand_id
FROM brands JOIN models USING (brand_id)
GROUP BY brand_id
HAVING COUNT(*) > 5;
...
FROM models FULL OUTER JOIN brands USING (brand_id)
